...
def get_recommendations(intent: str, novelty_level: int) -> list[dict]:
    user_prompt = (
        f"Intent: {intent}\n"
        f"Novelty level: {novelty_level} (see scale in system instructions)"
    )
    ...
    try:
        ...
        raw2 = _call_gemini(user_prompt)
        tracks = json.loads(raw2)

    if not isinstance(tracks, list):
        raise ValueError(f"Expected JSON array, got: {type(tracks)}")
